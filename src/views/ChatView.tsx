    ...
    useEffect(() => {
      ...
      useEffect(() => {
        ...
        const toggleRecording = async () => {
          if (isRecordingRef.current) {
            ...
            if (isLocalMode) {
              ...
              // Texten ligger redan i inputText - redo att redigeras
            } else {
            ...
            originalInputTextRef.current = inputText; // Spara det som eventuellt fanns i rutan
                ...
                if (data.matches && data.matches.length > 0) {
                  ...
                  if (autoSend) {
                    ...
                    silenceTimerRef.current = setTimeout(() => {
                      ...
                      handleSend(inputTextRef.current); // Skicka med den absolut senaste texten
                    }, 1500); // 1.5 sekunder tystnad triggar skick
                  }
                }
              });
        ...
        const handleSend = async (eOrText?: React.MouseEvent | React.KeyboardEvent | string) => {
          ...
          // Om vi håller på att spela in, avbryt inspelningen och skicka det vi har i rutan
          if (isRecordingRef.current) {
            isRecordingRef.current = false;
            setIsRecording(false);
            if (silenceTimerRef.current) clearTimeout(silenceTimerRef.current);
        ...
        return (
          ...
          placeholder={isRecording && isLocalMode ? "Lyssnar..." : "Skriv eller prata..."}
          ...
          <button onClick={handleSend} disabled={!inputText.trim() || isThinking || isTranscribing} className="p-3.5 bg-indigo-600 text-white rounded-full flex-shrink-0 disabled:opacity-50 disabled:bg-gray-300 transition-all shadow-md active:scale-95">
