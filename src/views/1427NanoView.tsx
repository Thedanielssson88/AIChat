...
const speakText = async (text: string, onEnded?: () => void) => {
  ...
  if (ttsMode === 'api' && openAiKey && openAiKey.trim().length > 0) {
    ...
    try {
      const voice = localStorage.getItem('TTS_VOICE') || 'onyx';
      ...
      if (!response.ok) throw new Error('OpenAI TTS API fel');
      ...
      console.error('OpenAI TTS misslyckades, faller tillbaka till Native TTS:', err);
    }
  }
  ...
  try {
    ...
    let ttsLang = langCode === 'en' ? 'en-US' : 'sv-SE';
    ...
    if ('speechSynthesis' in window) {
      ...
      const langCode = localStorage.getItem('TRANSCRIPTION_LANG') || 'sv';
      utterance.lang = langCode === 'en' ? 'en-US' : 'sv-SE';
...
export const NanoView = () => {
  ...
  const fetchMediaAsBlob = async (serverUrl: string, path: string) => {
    if (mediaCache.current[path]) return;
    mediaCache.current[path] = 'loading';
    try {
      ...
      reader.onloadend = () => {
        ...
        mediaCache.current[path] = dataUrl;
        setMediaCacheState(prev => ({ ...prev, [path]: dataUrl }));
      };
      reader.onerror = () => {
        mediaCache.current[path] = 'error';
        setMediaCacheState(prev => ({ ...prev, [path]: 'error' }));
